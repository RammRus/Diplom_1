...
class TestBurger:
    ...
    def test_add_ingredient(self, name, type_, price):
    ...
    def test_move_ingredients(self, from_idx, to_idx):
    ...
    def test_get_price(self, bun_name, bun_price, ing_name, ing_price):
        mock_bun = Mock(spec=Bun)
    ...
    def test_get_receipt(self):
        ...
        expected_receipt = burger.get_receipt()
        total_price = burger.get_price()
        receipt = (
            f"(==== {mock_bun.get_name} ====)\n"
            f"= {mock_ing1.get_type().lower()} {mock_ing1.get_name()}\n"
            f"= {mock_ing2.get_type().lower()} {mock_ing2.get_name()}\n"
            f"(==== {mock_bun.get_name} ====)\n"
            f"Price: {total_price}"
        )
        assert expected_receipt == receipt
        assert total_price == 2.9